...
const CardsRedirect: FC<RedirectProps> = ({ theming: { theme }, data, action = null, reset }) => {
  ...
  const redirect = (route: string, transactionType: string = '', name: string = '') => {
  ...
  return (
    ...
    <View style={styles.main}>
      <View style={styles.mainChild}>
        <TouchableOpacity onPress={() => redirect(data.type != 'FIAT' ? 'withdrawCryptoMain' : 'transactionType', 'withdraw', 'withdraw')}>
        ...
        <TouchableOpacity onPress={() => redirect(data.type != 'FIAT' ? 'receive' : 'transactionType', 'deposit', 'deposit')} >
          <View style={[styles.cardActions, { borderColor: data.color, backgroundColor: theme.defaultCard }]}>
            <View style={{ width: 24 }}>
